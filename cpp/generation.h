...
void Board::moveGen (vector<Move> &moves) {
    ...
    if (turn && wKingSqMoves==0) {
        if (wLRookSqMoves==0 && canLeftCastle()) {
            moves.push_back(Move(6, 60, 58, turn));
        }
        if (wRRookSqMoves==0 && canRightCastle()) {
            moves.push_back(Move(7, 60, 62, turn));
        }
    } else if (!turn && bKingSqMoves==0) {
        if (bLRookSqMoves==0 && canLeftCastle()) {
            moves.push_back(Move(6, 4, 2, turn));
        }
        if (bRRookSqMoves==0 && canRightCastle()) {
            moves.push_back(Move(7, 4, 6, turn));
        }
    }
    ...
    while (cPawns != 0) {
        ...
        while (attacks != 0) {
            moves.push_back(
                Move(0, lsb(cPawns), lsb(attacks), getCapturedPiece(lsb(attacks)), turn)
            );
          attacks &= attacks-1;
        }
        cPawns &= cPawns-1;
    ...
    //pawn forward 2
    while (up2 != 0) {
        moves.push_back(Move(0, lsb(up2) + (turn? 16 : -16), lsb(up2), turn));
        up2 &= up2-1;
    }
    ...
    while (cKnights != 0) {
        ...
        while (attacks != 0) {
            moves.push_back(
                Move(1, currSquare, lsb(attacks), getCapturedPiece(lsb(attacks)), turn)
            );
            attacks &= attacks-1;
        }
        while (openMoves != 0) {
            moves.push_back(Move(1, currSquare, lsb(openMoves), turn));
    ...
    while (up1 != 0) {
        ...
        if (turn? toSquare < 8 : toSquare > 55) {
            // moves.push_back(Move(8, turn? toSquare+8 : toSquare-8, toSquare, turn));
            // moves.push_back({9, fromSquare, toSquare, -1});
            // moves.push_back({10, fromSquare, toSquare, -1});
            moves.push_back(Move(11, turn? toSquare+8 : toSquare-8, toSquare, turn));
        } else {
            moves.push_back(Move(0, turn? toSquare+8 : toSquare-8, toSquare, turn));
        }
        up1 &= up1-1;
    }
    ...
    while (cBishops != 0) {
        ...
        while (attacks != 0) {
            moves.push_back(
                Move(2, lsb(cBishops), lsb(attacks), getCapturedPiece(lsb(attacks)), turn)
            );
            attacks &= attacks-1;
        }
        while (openMoves != 0) {
            moves.push_back(Move(2, lsb(cBishops), lsb(openMoves), turn));
            openMoves &= openMoves-1;
        }
        cBishops &= cBishops-1;
    }
    ...
    while (cQueen != 0) {
        ...
        while (attacks != 0) {
            moves.push_back(
                Move(4, currSquare, lsb(attacks), getCapturedPiece(lsb(attacks)), turn)
            );
            attacks &= attacks-1;
        }
        while (openMoves != 0) {
            moves.push_back(Move(4, currSquare, lsb(openMoves), turn));
            openMoves &= openMoves-1;
        }
        cQueen &= cQueen-1;
    }
    ...
    while (cRooks != 0) {
        ...
        while (attacks != 0) {
            moves.push_back(
                Move(3, lsb(cRooks), lsb(attacks), getCapturedPiece(lsb(attacks)), turn)
            );
          attacks &= attacks-1;
        }
        while (openMoves != 0) {
          moves.push_back(Move(3, lsb(cRooks), lsb(openMoves), turn));
          openMoves &= openMoves-1;
        }
        cRooks &= cRooks-1;
    }
    ...
    while (attacks != 0) {
        moves.push_back(
            Move(5, kingSquare, lsb(attacks), getCapturedPiece(lsb(attacks)), turn)
        );
        attacks &= attacks-1;
    }
    while (openMoves != 0) {
        moves.push_back(Move(5, kingSquare, lsb(openMoves), turn));
        openMoves &= openMoves-1;
    }
...
void Board::captureGen (vector<Move> &moves) {
    ...
    if (cKing) {
        ...
        while (attacks != 0) {
            ...
            moves.push_back(
                Move(5, currSquare, toSquare, getCapturedPiece(toSquare), turn)
            );
            attacks &= attacks-1;
        }
    } else {
    ...
    while (cPawns != 0) {
        ...
        while (attacks != 0) {
            moves.push_back(
                Move(
                    0, lsb(cPawns), lsb(attacks), getCapturedPiece(lsb(attacks)), turn)
                );
            attacks &= attacks-1;
        }
        cPawns &= cPawns-1;
    ...
    while (cKnights != 0) {
        ...
        while (attacks != 0) {
            moves.push_back(
                Move(
                    1, lsb(cKnights), lsb(attacks), getCapturedPiece(lsb(attacks)), turn)
                );
            attacks &= attacks-1;
        }
        cKnights &= cKnights-1;
